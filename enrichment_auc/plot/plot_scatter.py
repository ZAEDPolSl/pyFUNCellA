...
def show_predictions(
    ...
    fig, big_axes = plt.subplots(
        figsize=(12, nrows * 4), nrows=nrows, ncols=1, sharey=True
    )
    ...
    _ = sns.scatterplot(ax=ax1, x=tsne[0, s1_sign], y=tsne[1, s1_sign], color="#FF9700")

    ax2 = fig.add_subplot(gs[nrows - 2, 1], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(
        ax=ax2, x=tsne[0, s1_nsign], y=tsne[1, s1_nsign], color="#0C5AA6"
    )

    ax3 = fig.add_subplot(gs[nrows - 1, :], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(ax=ax3, x=tsne[0, :], y=tsne[1, :], hue=predicted)
    ...
    geneset_name = geneset_name.replace(":", "_")
    plt.savefig(
        save_dir + "/show_predictions_" + geneset_name + "_" + score_name + ".png"
    )
    if file_only:
        plt.close()
...
def plot_results(
    ...
    if predicted is None:
        show_labels(
            binary_predicted, true_labels, tsne, geneset_name, score_name, save_dir
        )
    else:
        show_predictions(
            binary_predicted,
...
def show_difference(
    ...
    both_significant = np.bitwise_and(
        np.asarray(score2_predicted), np.asarray(score1_predicted)
    ).astype(bool)
    score_2_sign = np.bitwise_and(
        np.asarray(score2_predicted), 1 - np.asarray(score1_predicted)
    ).astype(bool)
    score_1_sign = np.bitwise_and(
        1 - np.asarray(score2_predicted), np.asarray(score1_predicted)
    ).astype(bool)
    none_sign = np.bitwise_and(
        1 - np.asarray(score2_predicted), 1 - np.asarray(score1_predicted)
    ).astype(bool)
    ...
    if nrows == 3:
        ...
        ax1.set_ylim(np.min(tsne, axis=1)[1] - 1, np.max(tsne, axis=1)[1] + 1)
    _ = sns.scatterplot(
        ax=ax1, x=tsne[0, both_significant], y=tsne[1, both_significant], color="green"
    )

    ax2 = fig.add_subplot(gs[nrows - 2, 1], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(
        ax=ax2, x=tsne[0, score_1_sign], y=tsne[1, score_1_sign], color="red"
    )

    ax3 = fig.add_subplot(gs[nrows - 1, 0], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(
        ax=ax3, x=tsne[0, score_2_sign], y=tsne[1, score_2_sign], color="red"
    )

    ax4 = fig.add_subplot(gs[nrows - 1, 1], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(
        ax=ax4, x=tsne[0, none_sign], y=tsne[1, none_sign], color="green"
    )
...
def show_significance(
    ...
    ax3 = fig.add_subplot(gs[nrows - 1, 0], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(ax=ax3, x=tsne[0, s2_sign], y=tsne[1, s2_sign], color="#FF9700")

    ax4 = fig.add_subplot(gs[nrows - 1, 1], sharey=ax1, sharex=ax1)
    _ = sns.scatterplot(
        ax=ax4, x=tsne[0, s2_nsign], y=tsne[1, s2_nsign], color="#0C5AA6"
    )
