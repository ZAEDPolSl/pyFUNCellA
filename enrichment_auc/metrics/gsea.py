...
def calculate_gene_zscores(x, batch_size=30):
...
def rank_expressions(phenotype_expressions):
...
def _gsea(geneset, ranks, genes, gs_name=""):
...
def GSEA(genesets, ranks, genes):
    print("Calculating the score...\n")
    gsea = np.empty((len(genesets), ranks.shape[1]))
    for i, (gs_name, geneset_genes) in tqdm(
        enumerate(genesets.items()), total=len(genesets)
    ):
        gsea[i] = _gsea(geneset_genes, ranks, genes, gs_name)
    return gsea
