...
def blobs(n):
    def some_blobs():
        return make_blobs(
            n_samples=500, centers=np.reshape(n, (1, -1)), n_features=1, random_state=1
        )[0].ravel()
...
class TestFindThresholdsNative(unittest.TestCase):
    def test_separates_two_components(self):
        ...
        _, thresholds = ml.find_thresholds(values, max_components=2)
    ...
    def test_separates_multiple_components(self):
        ...
        _, thresholds = ml.find_thresholds(values, max_components=3)
...
slow_cases = [
    ...
    blobs([-2.388, -1.681, -1.495, 0.841, 0.920, 2.202, 2.914, 4.396, 7.694, 9.292]),
    blobs(
        [
            -2.684,
            -1.744,
    ...
    blobs([-5.421, -4.277, -2.010, -1.621, -0.376, 0.975, 3.481, 4.051, 4.269, 9.495]),
    blobs(
        [
            -5.587,
            -3.929,
...
cases_expectations = [
    ...
    [-5.141469013285568, -0.47571379978287176, 3.2771751168828303, 6.293933851327623],
    ...
    [-7.9103491430755355, -4.887751754416515, 1.8846776021326193, 7.127370908336459],
    [-7.786305907571306, -5.29228495409376, 2.7848138019657256, 7.2727379351231995],
]
