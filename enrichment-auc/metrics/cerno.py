...
def _cerno(geneset, data, genes, alpha=0.05, gs_name=""): # ordered gene names for each patient, geneset
    ...
    if N_tot <= N_gs or N_gs==0:
        ...
        return np.zeros(data.shape[1]), np.zeros(data.shape[1]), np.ones(data.shape[1]), np.ones(data.shape[1])
...
def CERNO(genesets, data, genes, alpha=0.05): # ordered gene names for each patient, genesets
    ...
    for i, (gs_name, geneset_genes) in tqdm(enumerate(genesets.items()), total=len(genesets)):
        cerno[i], auc[i], pval[i], qval[i] = _cerno(geneset_genes, data, genes, alpha, gs_name)
    return cerno, auc, pval, qval
