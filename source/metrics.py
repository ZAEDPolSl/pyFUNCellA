...
def _cerno(geneset, data, genes, gs_name=""): # ordered gene names for each patient, geneset
    genes_in_ds = [gene in geneset for gene in genes]
    in_gs = data[genes_in_ds, :]
    ...
    if N_tot <= N_gs or N_gs==0:
        ...
        return np.zeros(data.shape[1]), np.zeros(data.shape[1])
    R = np.sum(in_gs, axis=0)
    AUC = (N_gs*(N_tot-N_gs)+(N_gs+1)*N_gs/2 - R)/(N_gs*(N_tot-N_gs))
    cerno = 1 - stats.chi2.cdf(np.sum(np.log(in_gs/N_tot), axis=0)*(-2), 2*N_gs)
    return cerno, AUC

def CERNO(genesets, data, genes): # ordered gene names for each patient, genesets
    cerno = np.empty((len(genesets), data.shape[1]))
    auc = np.empty((len(genesets), data.shape[1]))
    for i, (gs_name, geneset_genes) in enumerate(genesets.items()):
        cerno[i], auc[i] = _cerno(geneset_genes, data, genes, gs_name)
    return cerno, auc
