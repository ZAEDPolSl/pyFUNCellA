  ...
  double *l_lik)
{
  ...
  double min_sig = calculate_SW(raw_sample, KS);
  /*  EM iterations  */
  /*  input data: */
  ...
  for (k = 0; k < nx; k++) {
    raw_sigvoc->data[k] = fmax(varargin_1->data[k], min_sig);
  }
  ...
  while ((change > 0.00015) && (rescue_iterations_count < 10000)) {
    ...
    for (nx = 0; nx < loop_ub; nx++) {
      ...
      if ((change < min_sig) || rtIsNaN(change)) {
        sigvoc->data[nx] = min_sig;
      } else {
